...
console.log("Receiver Address:", addressReceiver);
...
async function sendETH(toAddress, amount) {
  try {
    const transaction = await walletClient.sendTransaction({
      to: toAddress,
      value: parseEther(amount) // amount in ETH
    });
...
await checkBalance(addressReceiver);

// Example usage:
await sendETH(addressReceiver, '0.001'); 
